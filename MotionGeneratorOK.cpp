...
Eigen::VectorXf MotionGenerator::f(float t, Configuration& q){
	...
	//int mL = 0 + 6;  ////////// only SWING FOOT	
	//int mL = 3 + 0;  ////////// only COM position
	//int mL = 5 + 0;  ////////// only COM position + orientation
	int mL = 5 + 6;	
	...
	*/
	...
	/*
	// only COM position
	//Eigen::VectorXf td_CoM_i = prim->getzCoM()->getPosition(tk);
	//Eigen::VectorXf td_CoM_f = prim->getzCoM()->getPosition(tk + duration);
	//std::cout << "td_CoM_i ::" << std::endl;
	//std::cout << td_CoM_i.transpose() << std::endl;
	//std::cout << "td_CoM_f ::" << std::endl;
	//std::cout << td_CoM_f.transpose() << std::endl;
	JL = JCoM;
	tLc = tc_CoM.block(0,0,3,1);
	tLd = td_CoM.block(0,0,3,1);
	//tLd(0) = 0.0941413;	
	//tLd(1) = 0.00734113;	
	//tLd(2) = 0.814144;	
	tLd_dot = td_dot_CoM.block(0,0,3,1);
	Eigen::VectorXf eL = tLd - tLc;
	//eL(3) = angleSignedDistance(tLd(3), tLc(3));
	//eL(4) = angleSignedDistance(tLd(4), tLc(4));
	//eL(5) = angleSignedDistance(tLd(5), tLc(5));
	*/
	...
	/*
	// only COM position + orientation
	JL.block(0,0,3,dof) = JCoM;
	JL.block(3,0,2,dof) = JTorso.block(3,0,2,dof);
	tLc = tc_CoM.block(0,0,5,1); 
	tLd = td_CoM.block(0,0,5,1); 
	tLd_dot = td_dot_CoM.block(0,0,5,1);
	Eigen::VectorXf eL = tLd - tLc;
	eL(3) = angleSignedDistance(tLd(3), tLc(3));
	eL(4) = angleSignedDistance(tLd(4), tLc(4));
	*/
	...
	JL.block(0,0,3,dof) = JCoM;
	JL.block(3,0,2,dof) = JTorso.block(3,0,2,dof);
	JL.block(5,0,6,dof) = Jswg;
	tLc.block(0,0,5,1) = tc_CoM.block(0,0,5,1); 
	tLc.block(5,0,6,1) = tc_swg; 
	tLd.block(0,0,5,1) = td_CoM.block(0,0,5,1); 
	tLd.block(5,0,6,1) = td_swg; 
	tLd_dot.block(0,0,5,1) = td_dot_CoM.block(0,0,5,1);
	tLd_dot.block(5,0,6,1) = td_dot_swg;
	Eigen::VectorXf eL = tLd - tLc;
	eL(3) = angleSignedDistance(tLd(3), tLc(3));
	eL(4) = angleSignedDistance(tLd(4), tLc(4));
