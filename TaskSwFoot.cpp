...
TaskSwFoot::TaskSwFoot(int _type, float _duration, Configuration& qCurr, float _tk, float _height, Robot* robot, Kinematics* kin){
	...
	if(0.0 <= tk && tk < 0.6) supportFoot = L_FOOT;
	if(0.6 <= tk && tk < 1.2) supportFoot = R_FOOT;	
	if(1.2 <= tk && tk < 1.8) supportFoot = L_FOOT;	
	if(1.8 <= tk && tk < 2.4) supportFoot = R_FOOT;	
	if(2.4 <= tk && tk < 3.0) supportFoot = L_FOOT;	
	...
	if(!td.empty()){
		...
		while(it != td.end()){
			...
			if(!(t < ti-0.001) && t < tf-0.001){ 
				...
				if(t > 0.575) p_w(2) = (-4.0 * height) * (std::pow(((t - ti)/(tf - ti)), 2) - (((t - ti)/(tf - ti)))) + vswg_w(2);	
	...
	if(!td_dot.empty()){
		...
		while(it != td_dot.end()){
			...
			if(!(t < ti-0.001) && t < tf-0.001){ 
				...
				if(t > 0.575) v_w(2) = (-4.0 * height / (tf - ti)) * ((2.0 * ((t - ti)/(tf - ti))) - 1.0);
	...
	*/
	...
	//std::cout << "SWG taskPos.size():: " << taskPos.size() << std::endl; 
	//std::cout << "SWG taskVel.size():: " << taskVel.size() << std::endl; 
